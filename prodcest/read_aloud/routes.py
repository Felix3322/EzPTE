...
@会员专属
def 获取文本(file_id):
    路径 = os.path.join(文本目录, f"{file_id}.txt")
...
@朗读蓝图.route('/audio/<int:id>', methods=['GET'])
@login_required
@会员专属
def 获取音频(audio_id):
    路径 = os.path.join(音频目录, f"{audio_id}.mp3")
    if not os.path.exists(路径):
        abort(404, "音频文件不存在")
    return send_file(路径, mimetype='audio/mpeg')
